...
app.get('/api/weather', (req, res) => {
  if (NODE_ENV === 'dev') {
    res.send(JSON.parse(FAKE_WEATHER_RESP))
  }

  // api.openweathermap.org/data/2.5/forecast?lat=51.4532527&lon=-0.0659242&units=metric&appid=
  let options = {
    uri: 'https://api.openweathermap.org/data/2.5/weather',
    qs: {
      lat: req.param.lat || 51.4532527,
      lon: req.param.lon || -0.0659242,
      units: 'metric',
...
app.listen(port, () => console.log(`App listening on port ${port}! in ${NODE_ENV} mode`));
